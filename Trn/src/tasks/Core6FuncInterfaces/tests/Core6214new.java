...
public class Core6214new {
    abstract static class AbstractMessage<T> {
    ...
    public static class MailService<T> implements Consumer<AbstractMessage<T>> {

        private final Map<String, List<T>> mailMap = new HashMap<>() {
            @Override
        ...
        @Override
        public void accept(AbstractMessage<T> tAbstractMessage) {
            mailMap.computeIfAbsent(tAbstractMessage.getTo(), el -> new ArrayList<>()).add(tAbstractMessage.getContent());
        }
    ...
    public static void main(String[] args) {
        // Random variables
        String randomFrom = "bla bla bla"; // Некоторая случайная строка. Можете выбрать ее самостоятельно.
        String randomTo = "to bla bla";  // Некоторая случайная строка. Можете выбрать ее самостоятельно.
